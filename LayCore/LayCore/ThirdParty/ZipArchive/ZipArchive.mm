...
// KEEMI / Added by r.k.
-(BOOL) UnzipFileTo:(NSString*) path overWrite:(BOOL) overwrite andStateDelegate:(id<LayImportProgressDelegate>)stateDelegate {
    BOOL success = YES;
	...
	do{
        ...
        // KEEMI addedy by r.k. - get the current entry number
        if(stateDelegate) {
            unz_file_pos filePos = {0};
            int retVal = unzGetFilePos( _unzFile, &filePos );
            if(UNZ_OK == retVal) {
                NSUInteger currentEntryNumber = filePos.num_of_file;
                [stateDelegate setStep:currentEntryNumber];
            } else {
                MWLogError([ZipArchive class], @"Could not get the current number of the entry!");
            }
        }
...
{
    return [self UnzipFileTo:path overWrite:overwrite andStateDelegate:nil];
}
...
// KEEMI / added by r.k. to get the number if files in the zip
// This methof does not return the number of files zipped!!
// Instaed it returns the number of entries in the zip-file.
// A entry is not a file only but the parent directory to e.g.
/** 
- directeory1
 -- directeory1/file1.png
 -- directeory1/file2.png
-- directeory2
 */
// Furthermore there are hidden directoroes like the __MACOSX folder
-(NSUInteger)numberOfZippedEntries {
    NSUInteger numberOfZippedFiles = 0;
    unz_global_info  globalInfo = {0};
    if( unzGetGlobalInfo(_unzFile, &globalInfo )==UNZ_OK )
    {
        numberOfZippedFiles = globalInfo.number_entry;
    }
    
    return numberOfZippedFiles;
}


// KEEMI / added by r.k. copied from https://code.google.com/p/ziparchive/wiki/PageName
